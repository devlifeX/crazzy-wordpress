...
if ( ! is_admin() || bf_get_admin_current_post_type() !== 'page' || publisher_is_valid_cpt( 'post' ) ) {
	...
	$post_related_show_condition = [
		$show_condition,// this condition
		[   // or this condition
		    'post_related_type=custom-keyword'
		]
	];

	if ( publisher_get_option( 'post_related_type' ) === 'custom-keyword' ) {
		$post_related_show_condition[] = [ 'post_related_type=default' ];
	}
	...
	$fields['post_related_keyword'] = array(
		...
		'type'           => 'text',
		'show_on'        => $post_related_show_condition,
		'ajax-tab-field' => '_related_posts'
	);
	...
	//
	$ajaxified_related_count_show_condition = [
		['post_related=infinity-related-post']
	];
