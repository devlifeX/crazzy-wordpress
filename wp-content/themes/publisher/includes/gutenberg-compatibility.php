...
class Publisher_Gutenberg_Compatibility {
	...
	public function gallery_shortcode_compatible() {
		 ...
		 * FIX: wp>=5.7
		 * ============
		 * IN Admin: it should not unregister block because it got Uncaught react-dom error on edit page.
		 * In Front: it should unregister block otherwise custom gallery-view will not works.
		 */
		if ( ! is_admin() ) {
