...
if ( ! function_exists( 'publisher_shortcode_social_share_get_li' ) ) {
	...
	function publisher_shortcode_social_share_get_li( $id = '', $show_title = true, $count_label = 0 ) {
		...
		if ( bf_is_doing_ajax() || ( ! empty( $_REQUEST['action'] ) && ! empty( $_REQUEST['post_ID'] ) && 'ajax-get-post' === $_REQUEST['action'] ) ) {
		...
		switch ( $id ) {
			...
			case 'twitter':
				...
				$link  = 'https://twitter.com/share?text=' . esc_attr($page_title . $by) . '&url=' . $page_permalink;
